...
void Integrator::calculateEffectiveField(std::vector<std::vector<double>>& neighbours, double spin[constants::nAtoms * 3])
{
	for (int i = 0; i < constants::nAtoms; i++) {
		memcpy(&effectiveField[i], &constants::magneticField, sizeof(constants::magneticField)); //set to external field
...
void Integrator::rungeKutta(std::vector<std::vector<double>>& neighbours, double spin[constants::nAtoms * 3])
{
	double pDotp;
	double pDotB;
...
void Integrator::integrate(std::vector<std::vector<double>>& neighbours, double spin[constants::nAtoms * 3], double randomField[constants::nAtoms * 3])
{
